...
Output is handled line-by-line. An [`OutputLine`](#outputline) is modeled as a text string (`data`) with a property (`type`) that specifies its semantic meaning. These types, which are stored as an enum called [`OutputLineType`](#outputlinetype), include `NORMAL`, `MAJOR`, `MINOR`, `DEBUG`, and `SECTION_TITLE`.
